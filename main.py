...
##CONFIGURAÇÕES
####  TITULO DO MENU  #################################################################
title_menu = '[B][COLOR blue]:::[/COLOR][COLOR white]BEM-VINDOS AO [/COLOR][COLOR blue][B]Nordes[/B][/COLOR][COLOR white][B]Tv[/B][/COLOR][COLOR blue]:::[/COLOR][/B]'
###  DESCRIÇÃO DO ADDON ###############################################################
url_title_descricao = 'https://pastebin.com/raw/adERaPZ5'
####  LINK DO TITULO DE MENU  #########################################################
## OBS: POR PADRÃO JÁ TEM UM MENU EM BRANCO PARA NÃO TER ERRO AO CLICAR ###############
url_b64_title = 'aHR0cHM6Ly91cmxwbGF5LmNvbS9OMHJkTEI='
url_title = base64.b64decode(url_b64_title).decode('utf-8')

#### MENSAGEM BEM VINDOS  #############################################################
titulo_boas_vindas = 'BEM-VINDOS'
url_mensagem_bem_vindo = 'https://pastebin.com/raw/96i1Vspc'
####  MENSAGEM SECUNDARIA ######################################################
...
thumb_pesquisar = 'https://i.imgur.com/BgBWDf3.jpg'
fanart_pesquisar = 'https://i.imgur.com/224P6Fi.jpg'
mensagem_busca_invalida = 'Digite Algo!'
#### Descrição Pesquisa
url_desc_pesquisa = 'https://pastebin.com/raw/rmf2ebRP'
## MENU DOAÇÃO
menu_doacao = '[B][COLOR blue]DOAÇÃO[/COLOR][/B]'
thumb_icon_doacao = 'https://i.imgur.com/CVgJpLE.jpg'
desc_doacao = '[B][COLOR red]AVISO IMPORTANTE[/COLOR][/B]\n\nA Doação é para ajudar no projeto gratuito do addon que estamos fazendo para vocês, Quem fez doação acima de 10 tem direito a acesso ao grupo oficial WhatsApp, entre em contato e envie o comprovante.\n\nOBS: Sobre o PREMIUM é outro departamento, entre em contato.\n\n[COLOR blue]Luis Carlos[/COLOR]'
## MENU ATUALIZAÇÃO
menu_atualizacao = '[B][COLOR blue]ATUALIZAÇÃO[/COLOR][/B]'
thumb_update = 'https://i.imgur.com/IOl8aeF.jpg'
desc_atualizacao = 'Faça atualização automática no NordesTV usando esse recurso, só entrar que a atualização é verificada e atualizado.'
## MENU CONFIGURAÇÕES
menu_configuracoes = '[B][COLOR white]CONFIGURAÇÕES[/COLOR][/B]'
thumb_icon_config = 'https://i.imgur.com/N8TRzdR.jpg'
desc_configuracoes = 'Desativar ou ativar as configurações de notificações e a área PREMIUM do NordesTV'

## FAVORITOS
menu_favoritos = '[B][COLOR white]FAVORITOS[/COLOR][/B]'
thumb_favoritos = 'https://i.imgur.com/KeMkT5L.jpg'
desc_favoritos = 'Adicione Itens aos Favoritos, pressionando OK do controle ou clicando o direito e selecionando Adicionar aos favoritos do NordesTV'

#### MENU PREMIUM ################################################################
titulo_PREMIUM = '[B][COLOR blue]Nordes[/COLOR][COLOR white]TV[/COLOR][/B] [B][COLOR gold](PREMIUM)[/COLOR][/B]'
thumbnail_PREMIUM = 'https://i.imgur.com/1gU84D4.jpg'
fanart_PREMIUM = 'https://i.imgur.com/smdt7bf.jpg'
#### DESCRIÇÃO PREMIUM ###########################################################
url_PREMIUM_descricao = 'https://pastebin.com/raw/0jQBsZvv'
#### DIALOGO PREMIUM - SERVIDOR DESATIVADO - CLICK ###################################
url_PREMIUM_dialogo = 'https://pastebin.com/raw/9cJwsvn2'
##SERIVODR PREMIUM
url_server_PREMIUM = 'http://psrv.io:80/get.php'
...
useragent = base64.b32decode('aHR0cDovL25vcmRlc3R2LmdhYnNlcnYuY29tLmJyL1NlcnRhby9Ob3JkZXN0ZS9NZW51LVByaW5jaXBhbC5odG1s').decode('utf-8')

#Base
url_b64_principal = 'aHR0cDovL25vcmRlc3R2LmdhYnNlcnYuY29tLmJyL1NlcnRhby9Ob3JkZXN0ZS9NZW51LVByaW5jaXBhbC5odG1s'
...
if sys.argv[1] == 'limparEPG':
    ...
    xbmcgui.Dialog().ok('Sucesso', '[B][COLOR blue]EPG excluído com sucesso![/COLOR][/B]', 'Não se esqueça de salvar as configurações clicando em OK.')
    xbmc.sleep(2000)
    exit()
...
epg_download = base64.b64decode('aHR0cHM6Ly9wYXN0ZWJpbi5jb20vcmF3L2FDYWNFMmh1').decode('utf-8')
...
def resolve_data(url):
    if url.startswith(url_server_PREMIUM) == True:
...
#                print channel
            ...
            for other,channel_name,stream_url in match:
                ...
                try:
                    if url.startswith(url_server_PREMIUM) == True:
                        resolver_final = resolver_PREMIUM(stream_url, channel_name, thumbnail)
                    else:
    ...
    for item in items:
        ...
        try:
            if resolver(url, name, thumbnail).startswith('plugin://plugin.video.youtube/playlist') == True or resolver(url, name, thumbnail).startswith('plugin://plugin.video.youtube/channel') == True or resolver(url, name, thumbnail).startswith('plugin://plugin.video.youtube/user') == True or resolver(url, name, thumbnail).startswith('Plugin://plugin.video.youtube/playlist') == True:
                ...
                addDir2(name.encode('utf-8', 'ignore')+'[COLOR blue] ('+str(len(url2))+' itens)[/COLOR]'.encode('utf-8', 'ignore'),str(url2).replace(',','||').replace('$'+playlist_command+'','#'+playlist_command+''),11,str(subtitle2).replace(',','||'),cleaname,thumbnail,fanArt,desc.encode('utf-8'),False)
            elif len(url2) >1 and cleaname == '' and re.search(playlist_command,url,re.IGNORECASE):
                addDir2(name.encode('utf-8', 'ignore')+'[COLOR blue] ('+str(len(url2))+' itens)[/COLOR]'.encode('utf-8', 'ignore'),str(url2).replace(',','||').replace('$'+playlist_command+'','#'+playlist_command+''),11,subtitle,cleaname,thumbnail,fanArt,desc.encode('utf-8'),False)
...
def downloadEPG():
    try:
        ...
        notify('[COLOR blue]EPG atualizado com sucesso![/COLOR]',2000)
    except:
        notify('[COLOR red]Erro ao atualizar EPG![/COLOR][CR]Limpe o EPG e tente novamente.')
    ...
    try:
        ...
        for item in programas:
            ...
            if int(start) <= int(agora) < int(stop):
                epg = '\n[COLOR blue][B]' + start[8:-4] + ':' + start[10:-2] + '[/B] ' + item.find('title').text + '[/COLOR]'
                ...
                if cat is None or len(cat) == 0:
                    genero = '\n\n[COLOR white][B]GÊNERO:[/B][/COLOR] Desconhecido'
                elif len(cat) == 1:
                    genero = '\n\n[COLOR white][B]GÊNERO:[/B][/COLOR] '+ cat[0].text
                else:
                    genero = '\n\n[COLOR white][B]GÊNERO:[/B][/COLOR] '+ cat[0].text + '/' + cat[1].text
...
def rc_pro(channel):
    try:
        ...
        url = 'https://canaisgratis.org/'
        request = urllib2.Request(url, headers=request_headers)
        response = urllib2.urlopen(request)
        result_host = response.geturl()
    except:
        result_host = 'https://canaisgratis.org/'
...
def resolver_PREMIUM(link, name, thumbnail):
    link_decoded = link
    try:
        f4mPREMIUM = __addon__.getSetting('f4mPREMIUM')
    ...
    try:
        ...
        for i in items:
            ...
            try:
                ...
                urldecode = url
            try:
                urlrep = urldecode.replace('redtube.com/braziilian/NordesTV1',base64.b64decode('\x62\x32\x35\x6c\x63\x47\x78\x68\x65\x57\x68\x6b\x4c\x6d\x4e\x76\x62\x53\x39\x42\x59\x32\x56\x7a\x63\x32\x39\x51\x63\x6d\x39\x70\x59\x6d\x6c\x6b\x62\x79\x39\x42\x5a\x47\x52\x76\x62\x69\x39\x50\x62\x6d\x56\x51\x62\x47\x46\x35').decode('utf-8'))
            except:
                urlrep = urldecode
            mode = i[2]
            subtitle = i[3]
...
def addFavorite(name,url,fav_mode,subtitle,cleaname,iconimage,fanart,description):
    ...
    if os.path.exists(favorites)==False:
        ...
        a.close()
        notify('Adicionado aos Favoritos do NordesTV!')
...
def rmFavorite(name):
    ...
    for index in range(len(data)):
        if data[index][0]==name:
            ...
            b.close()
            break
    notify('Removido dos Favoritos do NordesTV!')
    xbmc.executebuiltin("XBMC.Container.Refresh")
...
def addDir(name,url,mode,iconimage,fanart,description,folder=True):
    ...
    if favoritos == 'true' and  mode !=4 and mode !=7 and mode !=8 and mode !=9 and mode !=10 and mode !=12 and mode !=15 and not url.startswith(url_title) and not url.find('username') >= 0 and not url.find('\x4d\x65\x6e\x75\x2d\x54\x56\x2e\x68\x74\x6d\x6c') >= 0 and not url.find('\x4d\x65\x6e\x75\x2d\x46\x69\x6c\x6d\x65\x73\x2e\x68\x74\x6d\x6c') >= 0 and not url.find('\x4d\x65\x6e\x75\x2d\x53\x65\x72\x69\x65\x73\x2e\x68\x74\x6d\x6c') >= 0 and not url.find('\x4d\x65\x6e\x75\x2d\x44\x65\x73\x65\x6e\x68\x6f\x73\x2e\x68\x74\x6d\x6c') >= 0 and not url.find('\x4d\x65\x6e\x75\x2d\x41\x6e\x69\x6d\x65\x73\x2e\x68\x74\x6d\x6c') >= 0 and not url.find('\x4d\x65\x6e\x75\x2d\x54\x6f\x6b\x75\x73\x61\x74\x73\x75\x2e\x68\x74\x6d\x6c') >= 0 and not url.find('\x4d\x65\x6e\x75\x2d\x4e\x6f\x76\x65\x6c\x61\x73\x2e\x68\x74\x6d\x6c') >= 0 and not url.find('\x4d\x65\x6e\x75\x2d\x52\x61\x64\x69\x6f\x73\x2e\x68\x74\x6d\x6c') >= 0 and not url.find('\x4d\x65\x6e\x75\x2d\x53\x68\x6f\x77\x73\x2e\x68\x74\x6d\x6c') >= 0 and not url.find('\x53\x65\x72\x69\x65\x73\x2d\x50\x72\x69\x6e\x63\x69\x70\x61\x6c\x2e\x68\x74\x6d\x6c') >= 0:
        try:
            url_fav = url.replace(base64.b64decode('bm9yZGVzdHYuY29tL0FjZXNzb1Byb2liaWRvL0FkZG9uL25vcmRlc3R2').decode('utf-8'),'redtube.com/braziilian/NordesTV1')
        except:
            url_fav = url
        try:
        ...
        try:
            ...
            if name_fav in FAV:
                try:
                    contextMenu.append(('Remover dos Favoritos do NordesTV','XBMC.RunPlugin(%s?mode=14&name=%s)'%(sys.argv[0], urllib.quote_plus(name))))
                except:
                    contextMenu.append(('Remover dos Favoritos do NordesTV','XBMC.RunPlugin(%s?mode=14&name=%s)'%(sys.argv[0], urllib.quote_plus(name.encode('utf-8', 'ignore')))))
            else:
                try:
                ...
                contextMenu.append(('Adicionar aos Favoritos do NordesTV','XBMC.RunPlugin(%s)' %fav_params))
...
def addLink(name,url,subtitle,cleaname,iconimage,fanart,description,folder=False):
    ...
    if favoritos == 'true':
        try:
            ...
            if name_fav in FAV:
                try:
                    contextMenu.append(('Remover dos Favoritos do NordesTV','XBMC.RunPlugin(%s?mode=14&name=%s)'%(sys.argv[0], urllib.quote_plus(name2_fav))))
                except:
                    contextMenu.append(('Remover dos Favoritos do NordesTV','XBMC.RunPlugin(%s?mode=14&name=%s)'%(sys.argv[0], urllib.quote_plus(name2_fav.encode('utf-8', 'ignore')))))
            else:
                try:
    ...
    try:
        ...
        if exists == True:
            ...
            if fcheck and fcheck.read() == '1' and check_addon == 'true':
                #print('valor 1')
                fcheck.close()
                link = getRequest2('https://raw.githubusercontent.com/luis45carlos/nordestvHD/master/nordestvRepo','').replace('\n','').replace('\r','')
...
def contador():
    try:
        ...
        "Referer": "NordesTV-3.0.5",
        "Connection": "close",
        }
        request = urllib2.Request("https://whos.amung.us/pingjs/?k=uqon2whfke", headers=request_headers)
...
def CheckUpdate(msg):
        try:
                uversao = urllib2.urlopen( "https://raw.githubusercontent.com/luis45carlos/RepoNordesTv/master/plugin.video.NordesTv/version.txt" ).read().replace('','').replace('','')
                ...
                if uversao != Versao:
                        ...
                        xbmcgui.Dialog().ok('[COLOR white][B]AVISO[/B][/COLOR]', "O addon já está atualizado na [COLOR blue]Versão: 4.0[/COLOR] em: "+Versao+"\nAs atualizações normalmente são automáticas caso não atualize baixe o add-on no site oficial.\n[COLOR blue][B]Use esse recurso caso não esteja recebendo automático.[/B][/COLOR]")
                        #xbmc.executebuiltin("XBMC.Container.Refresh()")
        except urllib2.URLError, e:
                if msg==True:
...
def Update():
        ...
        try:
                fonte = urllib2.urlopen( "https://raw.githubusercontent.com/luis45carlos/RepoNordesTv/master/plugin.video.NordesTvdatabase.py" ).read().replace('','')
                prog = re.compile('checkintegrity30122019').findall(fonte)
                if prog:
                        py = os.path.join( Path, "database.py")
                        file = open(py, "w")
                        file.write(fonte)
                        file.close()
                fonte = urllib2.urlopen( "https://raw.githubusercontent.com/luis45carlos/RepoNordesTv/master/plugin.video.NordesTv/downloader.py" ).read().replace('','')
                prog = re.compile('checkintegrity30122019').findall(fonte)
                if prog:
                        ...
                        file.write(fonte)
                        file.close()
                fonte = urllib2.urlopen( "https://raw.githubusercontent.com/luis45carlos/RepoNordesTv/master/plugin.video.NordesTv/downloader_epg.py" ).read().replace('','')
                prog = re.compile('checkintegrity30122019').findall(fonte)
                if prog:
                        ...
                        file.write(fonte)
                        file.close()
                fonte = urllib2.urlopen( "https://raw.githubusercontent.com/luis45carlos/RepoNordesTv/master/plugin.video.NordesTv/extract.py" ).read().replace('','')
                prog = re.compile('checkintegrity30122019').findall(fonte)
                if prog:
                        ...
                        file.write(fonte)
                        file.close()
                fonte = urllib2.urlopen( "https://raw.githubusercontent.com/luis45carlos/RepoNordesTv/master/plugin.video.NordesTv/main.py" ).read().replace('','')
                prog = re.compile('checkintegrity30122019').findall(fonte)
                if prog:
                        ...
                        file.write(fonte)
                        file.close()
                fonte = urllib2.urlopen( "https://raw.githubusercontent.com/luis45carlos/RepoNordesTv/master/plugin.video.NordesTv/resources/settings.xml" ).read().replace('','')
                ...
                if prog:
                        ...
                        file.write(fonte)
                        file.close()
                fonte = urllib2.urlopen( "https://raw.githubusercontent.com/luis45carlos/RepoNordesTv/master/plugin.video.NordesTv/addon.xml" ).read().replace('','')
...
def suporte():
    ...
    import webbrowser
    dialog = xbmcgui.Dialog()
    link = dialog.select('[B][COLOR white]SUPORTE[/COLOR] [B][COLOR blue]Nordes[/COLOR][COLOR white]TV[/COLOR][/B]', ['[B][COLOR blue]Nordes[/COLOR][COLOR white]TV[/COLOR][/B]: SITE', '[B][COLOR blue]Nordes[/COLOR][COLOR white]TV[/COLOR][/B]: REPOSITÓRIO','[B][COLOR blue]Nordes[/COLOR][COLOR white]TV[/COLOR][/B]: FACEBOOK','[B][COLOR blue]Nordes[/COLOR][COLOR white]TV[/COLOR][/B]: TELEGRAM','[B][COLOR blue]Nordes[/COLOR][COLOR white]TV[/COLOR][/B]: APP [COLOR blue](PREMIUM)[/COLOR] ENTRETENIMENTO','[B][COLOR blue]Nordes[/COLOR][COLOR white]TV[/COLOR][/B]: APP [COLOR blue](PREMIUM)[/COLOR] ELITE','[B][COLOR blue]Nordes[/COLOR][COLOR white]TV[/COLOR][/B]: [COLOR gold](P2P)[/COLOR] SOMENTE APLICATIVO','[B][COLOR blue]Nordes[/COLOR][COLOR white]TV[/COLOR][/B]: [B][COLOR white]ENTRAR NO ADDON[/COLOR][/B]'])
    if link == 0:
        if xbmc.getCondVisibility('system.platform.android'):
            xbmc.executebuiltin('StartAndroidActivity(,android.intent.action.VIEW,,%s)' %('http://nordestv.gabserv.com.br'))
        else:
            webbrowser.open('http://nordestv.gabserv.com.br')
    if link == 1:
        if xbmc.getCondVisibility('system.platform.android'):
            xbmc.executebuiltin('StartAndroidActivity(,android.intent.action.VIEW,,%s)' %('http://nordestv.gabserv.com.br/nordestv'))
        else:
            webbrowser.open('http://nordestv.gabserv.com.br/nordestv')
    if link == 2:
        if xbmc.getCondVisibility('system.platform.android'):
            xbmc.executebuiltin('StartAndroidActivity(,android.intent.action.VIEW,,%s)' %('https://www.facebook.com/groups/NordesTV&Brlive' ))
        else:
            webbrowser.open('https://www.facebook.com/groups/NordesTV&Brlive')
    if link == 3:
        if xbmc.getCondVisibility('system.platform.android'):
            xbmc.executebuiltin('StartAndroidActivity(,android.intent.action.VIEW,,%s)' %('https://t.me/NordesTV' ))
        else:
            webbrowser.open('https://t.me/NordesTV')
    if link == 4:
        if xbmc.getCondVisibility('system.platform.android'):
            xbmc.executebuiltin('StartAndroidActivity(,android.intent.action.VIEW,,%s)' %('http://nordestv.gabserv.com.br/apk/Nordestv-Entretenimento.apk'))
        else:
            webbrowser.open('http://nordestv.gabserv.com.br/apk/Nordestv-Entretenimento.apk')
    if link == 5:
        if xbmc.getCondVisibility('system.platform.android'):
            xbmc.executebuiltin('StartAndroidActivity(,android.intent.action.VIEW,,%s)' %('http://nordestv.gabserv.com.br/apk/Nordestv-Elite.apk'))
        else:
            webbrowser.open('http://nordestv.gabserv.com.br/apk/Nordestv-Elite.apk')
    if link == 6:
        if xbmc.getCondVisibility('system.platform.android'):
            xbmc.executebuiltin('StartAndroidActivity(,android.intent.action.VIEW,,%s)' %('https://stanx.me/hx'))
        else:
            webbrowser.open('https://stanx.me/hx')
...
def doacao():
    ...
    import webbrowser
    dialog = xbmcgui.Dialog()
    link = dialog.select('[B][COLOR white]FAÇA UMA DOAÇÃO AO[/COLOR] [B][COLOR blue]Nordes[/COLOR][COLOR white]TV[/COLOR][/B]', ['[B][COLOR blue]Nordes[/COLOR][COLOR white]TV[/COLOR][/B]: DOAÇÃO [COLOR steelblue]MERCADO[/COLOR][COLOR skyblue]PAGO[/COLOR] R$10,00','[B][COLOR blue]Nordes[/COLOR][COLOR white]TV[/COLOR][/B]: DOAÇÃO [COLOR steelblue]MERCADO[/COLOR][COLOR skyblue]PAGO[/COLOR] R$10,00','[B][COLOR blue]Nordes[/COLOR][COLOR white]TV[/COLOR][/B]: DOAÇÃO [COLOR steelblue]MERCADO[/COLOR][COLOR skyblue]PAGO[/COLOR] R$10,00','[B][COLOR blue]Nordes[/COLOR][COLOR white]TV[/COLOR][/B]: DOAÇÃO [COLOR lime]PYCPAY[/COLOR]','[B][COLOR blue]Nordes[/COLOR][COLOR white]TV[/COLOR][/B]: DOAÇÃO [COLOR lime]PYCPAY[/COLOR]','[B][COLOR blue]Nordes[/COLOR][COLOR white]TV[/COLOR][/B]: DOAÇÃO [COLOR white]PAG[/COLOR][COLOR lime]SEGURO[/COLOR]','[B][COLOR blue]Nordes[/COLOR][COLOR white]TV[/COLOR][/B]: DOAÇÃO [COLOR white]COM ANÚNCIO[/COLOR]','[B][COLOR blue]Nordes[/COLOR][COLOR white]TV[/COLOR][/B]: [B][COLOR white]CONTINUAR NO ADDON[/COLOR][/B]'])
    if link == 0:
        if xbmc.getCondVisibility('system.platform.android'):
            xbmc.executebuiltin('StartAndroidActivity(,android.intent.action.VIEW,,%s)' %('https://www.mercadopago.com.br/checkout/v1/redirect?pref_id=395946085-4ad498e7-9d75-4b0e-aede-c2171713f23c' ))
        else:
            webbrowser.open('https://www.mercadopago.com.br/checkout/v1/redirect?pref_id=395946085-4ad498e7-9d75-4b0e-aede-c2171713f23c')
    if link == 1:
        if xbmc.getCondVisibility('system.platform.android'):
            xbmc.executebuiltin('StartAndroidActivity(,android.intent.action.VIEW,,%s)' %('https://www.mercadopago.com.br/checkout/v1/redirect?pref_id=395946085-4ad498e7-9d75-4b0e-aede-c2171713f23c' ))
        else:
    ...
    if link == 4:
        if xbmc.getCondVisibility('system.platform.android'):
            xbmc.executebuiltin('StartAndroidActivity(,android.intent.action.VIEW,,%s)' %('https://app.picpay.com/user/luiscarlos45' ))
        else:
            webbrowser.open('https://app.picpay.com/user/luiscarlos45')
    if link == 5:
        if xbmc.getCondVisibility('system.platform.android'):
            xbmc.executebuiltin('StartAndroidActivity(,android.intent.action.VIEW,,%s)' %('https://pag.ae/7WjDTisg7' ))
        else:
            webbrowser.open('https://pag.ae/7WjDTisg7')
    #pagina anuncio
    if link == 6:
        if xbmc.getCondVisibility('system.platform.android'):
            xbmc.executebuiltin('StartAndroidActivity(,android.intent.action.VIEW,,%s)' %('http://nordestv.gabserv.com.br/' ))
...
def PREMIUM():
    username_PREMIUM = addon.getSetting('username')
    password_PREMIUM = addon.getSetting('password')
    #tipo_servidor = addon.getSetting('servidor')
    PREMIUM_menu = addon.getSetting('exibirPREMIUM')
    saida_transmissao = addon.getSetting('saida')
    if PREMIUM_menu == 'true':
        #if tipo_servidor=='NordesTV':
        if username_PREMIUM > '' and password_PREMIUM > '':
            url = ''+url_server_PREMIUM+'?username=%s&password=%s&type=m3u_plus&output=%s'%(username_PREMIUM,password_PREMIUM,saida_transmissao)
            #addDir(name,url,mode,iconimage,fanart,description)
            addDir(titulo_PREMIUM,url,1,thumbnail_PREMIUM,fanart_PREMIUM,getRequest2(url_PREMIUM_descricao, ''))
        else:
            #if tipo_servidor=='Desativado':
            #addDir(name,url,mode,iconimage,fanart,description)
            addDir(titulo_PREMIUM,'',9,thumbnail_PREMIUM,fanart_PREMIUM,getRequest2(url_PREMIUM_descricao, ''))
...
#Link Vazio
    ...
    xbmcgui.Dialog().ok(titulo_PREMIUM, getRequest2(url_PREMIUM_dialogo, ''))
    xbmcaddon.Addon().openSettings()
    xbmcgui.Dialog().ok('[B][COLOR white]AVISO[/COLOR][/B]','FECHE O KODI E ABRA NOVAMENTE PARA ATUALIZAR AS CONFIGURAÇÕES')
    xbmc.executebuiltin("XBMC.Container.Refresh()")
